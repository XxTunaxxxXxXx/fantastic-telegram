...
if __name__ == "__main__":     
    forzaData = Telemetry(port_number)  #initialize server with set port
    print(f"Local IP address is : {forzaData.local_ip}")   
    print(f"\nListening port is : {forzaData.port}")  
    input("Press enter to continue")
    screen_clear()
    forzaData.startServer() #start listening to for incoming data
    print ('[WAITING TO RECIEVE DATA FROM FORZA')
    forzaData.dataRefresh()
